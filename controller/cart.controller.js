...
const addToCart = async (req, res) => {
  ...
  try {
    ...
    const cartItem=await CartModel.findOne({userID:req._id,product:productID})
    const {name,image,price1,category,quantity,price2,discount,star_rating,userID}=product
    ...
    if(!cartItem){
      if (Check(product.quantity, reqQuantity)) {
        ...
        await ProductModel.findByIdAndUpdate({_id:productID},{$inc:{quantity:-1}})
      ...
      if(!type){
        ...
        if (Check(product.quantity, reqQuantity)) {
          ...
          await CartModel.findOneAndUpdate({product:productID},{$inc:{quantity:1}})
          await ProductModel.findByIdAndUpdate({_id:productID},{$inc:{quantity:-1}})
          return res.status(200).send({msg:"Cart has been updated succussfully"})
        }
        ...
        if(cartItem.quantity==1){
          ...
          const cart=await CartModel.findOneAndUpdate({product:productID},{$inc:{quantity:-1}})
...
const deleteCartItem=async(req,res)=>{
  try {
    ...
    const product=await ProductModel.findByIdAndUpdate({_id:cart.product},{$inc:{quantity:cart.quantity}})
